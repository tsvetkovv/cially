...
module.exports = {
  ...
  execute(message) {
    ...
    // FIXME fix dis
    const imgs = message.attachments.map(img => img)
    console.log(imgs)
    ...
    // HTTP
    let info = {"guildID": message.guild.id, "messageID": message.id, "messageLength": totalWords.length, "channelID": message.channelId, "authorID": message.author.id}
    sendPostRequest({data: info, guildId: message.guild.id, type: module.exports.name})
    }
  },
}