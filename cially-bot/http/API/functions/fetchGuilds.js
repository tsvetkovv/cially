    ...
    try {
      ...
      discord_guilds.forEach(async guild => {
        ...
        if (guilds_in_database.includes(guild.id)) {
          await guildsArray.push({ name: guild.name, id: guild.id, icon: icon, in_db: true })
        } else {
          await guildsArray.push({ name: guild.name, id: guild.id, icon: icon, in_db: false })
        }
      ...
      error({ text: `Failed to communicate with the Discord API. /fetchGuilds` });
      console.log(err)
      res.send(error_message);

    }
  } catch (err) {
    error({ text: `Failed to communicate with the PocketBase Instance. /fetchGuilds` });
    console.log(err)
    res.send(error_message);
